...
const TierServiceFirebase = require('./tier-service-firebase');
...
class MintService {
    constructor() {
        ...
        // Services
        this.tierService = new TierServiceFirebase();
    ...
    async mintByRarity(userAccountId, rarity, quantity = 1) {
        ...
        try {
            ...
            // ✅ ADD: Check association FIRST
            console.log(`\n🔍 PHASE 0: Checking token association...`);
            const isAssociated = await this.checkAssociation(userAccountId);

            if (!isAssociated) {
                throw new Error(`Token not associated. User ${userAccountId} must associate with token ${this.tokenId} before minting.`);
            }
